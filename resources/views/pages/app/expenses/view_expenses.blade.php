<x-base-layout :scrollspy="false">
    ...
    <x-slot:headerFiles>
        ...
        @foreach($projet->entreesSorties as $entreeSortie)
            <div class="modal fade" id="editProjectModal{{  $entreeSortie->id }}" tabindex="-1" aria-labelledby="createProjectModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    ...
                    @endif
                            ...
                            <form action="{{ route('expenses.update', $entreeSortie->id) }}" method="POST" enctype="multipart/form-data">
                                @method('PUT')
                                @csrf
                                            ...
                                            <div class="row mb-2">
                                                <div class="col-sm-12">
                                                    <select name="id_projet" class="form-select form-select-sm @error('id_projet') is-invalid @enderror" aria-label=".form-select-sm example" required>
                                                        ...
                                                        @foreach($allprojets as $data)
                                                            <option value="{{ $data->id }}" {{ $entreeSortie->id_projet == $data->id ? 'selected' : '' }}>{{ $data->nom }}</option>
                                                        @endforeach
                                                    </select>
                                                    @error('id_projet')
                                                        <span class="invalid-feedback" role="alert">
        ...
        {{-- @if($entreesSorties->isEmpty())
        <p>Aucune Entrée/Sortie n'a été trouvé.</p>
    @else --}}
                            ...
                            <center><br>
                                <div class="row">
                                    <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                                        <h4>Liste des dépenses liées au Projet <b>{{ $projet->nom }}</b></h4>
                                    </div>
                                </div>
                            </center>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <span style="font-size: 55%" class="shadow-none badge badge-{{ $entreeSortie->status == 0 ? 'success' : 'warning' }}">
                                                {{ $entreeSortie->status == 0 ? 'Entrée' : 'Sortie' }}
                                            </span>
                                        </td>
                                        <td class="text-center">
                                            @if($entreeSortie->file)
                                                <a href="{{ asset('uploads/EntreeSortieProjet/' . $entreeSortie->file) }}" target="_blank">
                                                    <i class="las la-eye"></i>
                                                </a>
                                                <a href="{{ asset('uploads/EntreeSortieProjet/' . $entreeSortie->file) }}" download>
                                        ...
                                        <td class="text-center">
                                            <div class="d-grid gap-2 d-md-block">
                                                ...
                                                <button style="font-size: 65%" class="delete-button btn btn-danger btn-sm _effect--ripple waves-effect waves-light warning confirm" data-delete-url="{{ route('expenses.destroy', $entreeSortie->id) }}">Supprimer</button>
